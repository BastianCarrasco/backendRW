...
// Obtener todos los postualciones
app.get('/postulaciones', async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT * FROM postulaciones ORDER BY id');
    res.json(rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Error al obtener fondos' });
...
app.post('/postulaciones', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      'INSERT INTO postulaciones(link) VALUES($1) RETURNING *',
      [link]
    );
...
app.delete('/postulaciones/:id', async (req, res) => {
  ...
  try {
    const { rowCount } = await pool.query('DELETE FROM postulaciones WHERE id = $1', [id]);
